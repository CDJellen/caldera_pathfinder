...
class PathfinderService:
    ...
    async def import_scan(self, scan_format, report):
        ...
        try:
            ...
            if parsed_report:
                report = await self.enrich_report(parsed_report) or parsed_report
                await self.data_svc.store(report)
                return await self.create_source(report)
            return None
        finally:
            os.remove(temp_file)
    ...
    async def collect_tagged_abilities(self, ability_tags: list[str]) -> list:
        ...
        Args:
            ability_tags: A list of string-valued tags on which to match caldera
                          abilities.

        Returns:
            A list of all abilities matching one or more tags, as found by the
            data service, returns an empty list if no tagged abilities are
            found.
        """
        return [
            a
    ...
    async def collect_tagged_adversaries(self, adversary_tags: list[str]
        ...
        Args:
            adversary_tags: A list of string-valued tags on which to match
                            caldera abilities.

        Returns:
            A list of all adversaries matching one or more tags, as found by the
            data service, returns an empty list if no tagged adversaries are
            found.
        """
        return [
            a.display
    ...
    async def build_graph(self, report: 'VulnerabilityReport'
                          ) -> 'nx.MultiDiGraph':
        """Build the Pathfinder attack graph from vuln report nodes and edges.
        ...
        Returns:
            A networkx `MultiDiGraph` in which each node is connected to other
            nodes based on the report `network_map_edges`, and each edge is
            named using the uuid of the applicable Pathfinder adversary which
            could enable compromise of the target node from the source node.
            Both the `weigh`, computed as the inverse of the adversary
            `success_prob`, and the number of abilities in the adversary are
            recorded in each edge dictionary.
        """
        multi_dict = dict()
    ...
    async def get_paths(self, report: 'VulnerabilityReport', initial_host: str,
        ...
        for i, path in enumerate(paths):
            for edge in path:
                ...
                sorted_paths[i].append(
                    (src, tgt, adv, weights['weight'], weights['steps'])
                    )
        path_data['paths'] = paths
        path_data['sorted_paths'] = sorted_paths
        return path_data

    async def generate_adversary_v2(self, path: list, tags: list = []):
        """ TODO """
        async def create_cve_adversary(techniques: list, tags: list) -> dict:
            """ TODO """
            adv_id = uuid.uuid4()
            obj_default = (
                await self.data_svc.locate('objectives', match=dict(name='default'))
